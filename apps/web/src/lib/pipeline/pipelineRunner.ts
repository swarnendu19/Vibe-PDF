 ...
 *
 * Source: .agent/AI_PIPELINE.md, .agent/AGENT_RULES.md
 */
...
export async function runPipeline(
  ...
  try {
    // ── STAGE 1: PLANNING ────────────────────────────────────────────────────
    updateJob(registry, jobId, { currentStage: 'PLANNING', status: 'RUNNING', progress: 2 })
    ...
    updateJob(registry, jobId, { progress: progressAt('PLANNING') })

    // ── STAGE 2: RESEARCHING ─────────────────────────────────────────────────
    updateJob(registry, jobId, { currentStage: 'RESEARCHING', progress: progressAt('PLANNING') + 2 })
    ...
    updateJob(registry, jobId, { progress: progressAt('RESEARCHING') })

    // ── STAGE 3: OUTLINING ───────────────────────────────────────────────────
    updateJob(registry, jobId, { currentStage: 'OUTLINING', progress: progressAt('RESEARCHING') + 2 })
    ...
    updateJob(registry, jobId, { progress: progressAt('OUTLINING') })

    // ── STAGE 4: WRITING ─────────────────────────────────────────────────────
    updateJob(registry, jobId, { currentStage: 'WRITING', progress: progressAt('OUTLINING') + 2 })
    ...
    updateJob(registry, jobId, { progress: progressAt('WRITING') })

    // Parse the document AST from the writer output
    const documentAST = parseDocumentAST(rawAST, job)

    // ── STAGE 5: ILLUSTRATING ────────────────────────────────────────────────
    updateJob(registry, jobId, { currentStage: 'ILLUSTRATING', progress: progressAt('WRITING') + 2 })

    const illustratedAST = await runIllustrationAgent(documentAST)

    updateJob(registry, jobId, { progress: progressAt('ILLUSTRATING') })

    // ── STAGE 6: LAYOUT ──────────────────────────────────────────────────────
    updateJob(registry, jobId, { currentStage: 'LAYOUT', progress: progressAt('ILLUSTRATING') + 2 })

    const laidOutAST = applyLayoutVariants(illustratedAST)

    updateJob(registry, jobId, { progress: progressAt('LAYOUT') })

    // ── STAGE 7: QA ──────────────────────────────────────────────────────────
    updateJob(registry, jobId, { currentStage: 'QA', progress: progressAt('LAYOUT') + 2 })
    ...
    updateJob(registry, jobId, { progress: progressAt('QA') })

    // ── STAGE 8: EXPORTING ───────────────────────────────────────────────────
    updateJob(registry, jobId, { currentStage: 'EXPORTING', progress: progressAt('QA') + 2 })

    // Mark as complete — PDF export happens on-demand from canvas
    updateJob(registry, jobId, {
      status: 'COMPLETE',
...
function parseDocumentAST(rawText: string, job: JobRecord): DocumentAST {
  // Try to extract JSON block from the writer's output
  ...
  // Fallback: synthesize a structured document from plain text
...
function synthesizeDocumentFromText(
  ...
  // Extract title from first line or prompt
...
function buildContentPage(section: Section, pageNumber: number): PageNode {
  ...
  return {
    id: crypto.randomUUID(),
    pageNumber,
    layoutVariant: section.hasData ? 'standard_editorial' : 'standard_editorial',
    elements,
  }
}

// ─────────────────────────────────────────────────────────────────────────────
// ILLUSTRATION AGENT (generates SVG descriptions)
// ─────────────────────────────────────────────────────────────────────────────
...
function runQualityGate(doc: DocumentAST): number {
  ...
  // Must have at least cover + 1 content page
  if (doc.pages.length < 2) score -= 30

  // Each page needs at least 2 elements
